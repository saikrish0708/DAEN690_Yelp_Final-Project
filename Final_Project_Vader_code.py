...
while(count< n):
    ...
    if vs['compound'] >0.5:
        ...
        print("negative")
    d.append({'Business ID':reviews['business_id'][count], 'Vader Score' : vs['compound']})    
    count+=1
d    
type(d)
df=pd.DataFrame(d)
df
new_df=df.groupby('Business ID')['Vader Score'].agg(['mean'])
new_df.to_csv('FinalProject_690.csv', encoding='utf-8', index=True)

new=(df.groupby('Business ID')
   .....:    .agg({'Vader Score':'mean'})
   .....:    .reset_index()
   .....: )
new.to_csv('FinalProject_690_1.csv', encoding='utf-8', index=True)
